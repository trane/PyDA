...
def normalize(pda):
    ...
    delta = set()
    for init,inputs in pda['Delta'].items():      # state: input
        for inpt,stack in inputs.items():         # input: top of stack
            for stack_t,dest in stack.items():    # top of stack: dest state
                for state,pushes in dest.items(): # state: [push1, push2]
                    for push in pushes:
                        delta.add((init, inpt, stack_t, state, push))
    npda['Delta'] = delta
    npda['F'] = set(pda['F'])
    npda['q0'] = pda['q0']
    npda['Z'] = pda['Z']
