...
involved. The first thing we had to do is define exactally what this program should
be able to do, and then provid the user with a very straighforward way of doing it.
We opted to go with a model view controller (MVC) type of approach to seperate 
out the parts of this program. The npda object acted as the model,, and the 
PyDA\_cli.py acted as the view and controller. I decided the best way to do 
this would be to set up an event loop that processed user input, simaler to 
a shell, or gdb. After we had defined everything that this program needed to do,
it was a fairly straighforward process to parse the users commands and use it
